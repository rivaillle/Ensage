...
namespace MySnatcher
{
    internal class Program
    {
        ...
        private static void Game_OnUpdate(EventArgs args)
        {
            ...
            {
                var rune = ObjectManager.GetEntitiesFast<Rune>().FirstOrDefault(x => x.IsVisible && x.Distance2D(me) < 350);
                var aegis = ObjectManager.GetEntitiesFast<PhysicalItem>().FirstOrDefault(x => x.IsVisible && x.Distance2D(me) < 380 && x.Item.Name == "item_aegis");
                if (rune != null)
                {
                    me.PickUpRune(rune);
                    return;
                }
                if (aegis != null && me.Inventory.FreeInventorySlots.Any())
                {
                    me.PickUpItem(aegis);
                    return;
                }
